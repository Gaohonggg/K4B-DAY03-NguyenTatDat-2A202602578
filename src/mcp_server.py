...
if __name__ == "__main__":
    ...
    # Kiểm tra Tool Schemas VinBus
    expected_tools = {"route_lookup", "register_monthly_pass"}
    published_tools = {tool.get("name") for tool in tools}
    if published_tools != expected_tools:
        print(f"⚠️ [TOOL SCHEMA]: Danh sách tool chưa đúng: {sorted(published_tools)}")
    else:
        print("✅ [TOOL SCHEMA]: Hai VinBus tool đã có schema đầy đủ.")

    # Kiểm tra trạng thái TODO 2.1 (call_tool)
    test_result = server.call_tool("route_lookup", {"query": "V01"})
    if not test_result:
        print("⏳ [TODO 2.1]: Hàm call_tool() đang trả về rỗng. Học viên hãy hoàn thiện TODO 2.1 trong 'src/mcp_server.py'!")
    else:
        print(f"✅ [TODO 2.1]: Test dispatch tool 'route_lookup' thành công:")
